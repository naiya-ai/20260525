...
ROOT="${ROOT:-model_experiment/classification_2013_2024_single_lr3e-4_10k_bs512}"
...
launch_one() {
  ...
  local seed="$((5000 + width))"
  echo "==> launch width=${width} on GPU ${gpu}, dataset=${DATASET_NAME}, batch=${BATCH_SIZE}, lr=${LR}, steps=${STEPS}"
  (

...
CONFIG="${CONFIG:-configs/train/train_raw_mlp_disease_classifier_diabetes_2013_2024_year_stratified.yaml}"
ROOT="${ROOT:-model_experiment/classification_2013_2024_year_stratified_single_lr3e-4_100k_bs512}"
...
launch_one() {
  ...
  local seed="$((6000 + width))"
  echo "==> launch width=${width} on GPU ${gpu}, batch=${BATCH_SIZE}, lr=${LR}, steps=${STEPS}, class_weight=${CLASS_WEIGHT}"
  (

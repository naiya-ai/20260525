...
SWEEP_ID="${SWEEP_ID:-cvae_z16_beta010_compact_questionnaire_6diseases_hold500_5x_$(date +%Y%m%d_%H%M%S)}"

BETAS="${BETAS:-0.1}" \
DISEASES="${DISEASES:-diabetes hypertension dyslipidemia liver_disease kidney_disease anemia}" \
REPEATS="${REPEATS:-5}" \
GPUS="${GPUS:-0 1 2 3 4 5 6 7}" \
CONFIG="${CONFIG:-configs/train/train_conditional_vae_eddi_z16_beta010_compact_questionnaire_beta_hold500.yaml}" \

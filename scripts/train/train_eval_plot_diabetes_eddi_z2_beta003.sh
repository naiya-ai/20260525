...
RUN_NAME="${RUN_NAME:-eddi_z2_beta003_diabetes_$(date +%Y%m%d_%H%M%S)}"
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/conditional_vae_eddi_z2_beta003/${RUN_NAME}}"
EVAL_ROOT="${EVAL_ROOT:-outputs/eval_eddi_z2_beta003/${RUN_NAME}/harmonized_knhanes_1998_2024}"
...
./scripts/train/train_diabetes_eddi_z2_beta003.sh
...
PROBABILITIES="${EVAL_ROOT}/eddi_z2_beta003_best_${DISEASE}_probabilities.csv"
AUROC_JSON="${EVAL_ROOT}/eddi_z2_beta003_best_${DISEASE}_auroc.json"
...
PYTHONPATH=src uv run python scripts/analysis/plot_latent_prior_posterior_heatmaps.py \
  ...
  --output "${PLOT_ROOT}/${DISEASE}_eddi_z2_beta003_latent_prior_posterior_heatmaps.png" \
  --title "Diabetes EDDI z2 beta=0.003 latent prior/posterior on test samples"

echo "==> AUROC: ${AUROC_JSON}"
echo "==> Heatmap: ${PLOT_ROOT}/${DISEASE}_eddi_z2_beta003_latent_prior_posterior_heatmaps.png"

...
echo "==> Transformer CVAE run: ${RUN_NAME}"
...
PARALLEL=1 \
SKIP_EXISTING=0 \
./scripts/train/train_transformer_cvae_diseases.sh "${DISEASES[@]}"
mkdir -p "${EVAL_ROOT}"
...
variant_order = ["catboost", "cvae", variant]
...
fig.suptitle("Disease Prediction Metrics", fontsize=14)
fig.tight_layout()
